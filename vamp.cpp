...
//
vamp::vamp(int N, int M,  int Mt, double gam2, double gamw, int max_iter, double rho, std::vector<double> vars,  std::vector<double> probs, std::vector<double> true_signal, int rank, std::string out_dir, std::string out_name, std::string model, Options opt) :
    N(N),
    M(M),
    Mt(Mt),
    C(opt.get_C()),
...
//
vamp::vamp(int M, double gam2, double gamw, std::vector<double> true_signal, int rank, Options opt):
    M(M),
    C(opt.get_C()),
    gam2(gam2),
    gamw(gamw),
...
std::vector<double> vamp::infere( data* dataset ){
    ...
    // saving msig_people
    std::string filepath_out_msig_people = out_dir + out_name + "_msig_people.csv";
    store_vec_to_file(filepath_out_msig_people, msig_people);
    if (rank == 0)
        std::cout << "msig_people filepath_out is " << filepath_out_msig_people << std::endl;

    double max = *max_element(msig_people.begin(), msig_people.end());
    double min = *min_element(msig_people.begin(), msig_people.end());
    if (rank == 0)
        std::cout << "max = " << max << ", min = " << min << std::endl;

    //double *msig = (*dataset).get_msig();
    //int nonas = (*dataset).get_nonas();

    //double trace_XXT = 0;
    //for (int i=0; i<M; i++){
    //    trace_XXT += (nonas - 1) / msig[i]*msig[i];
    //}

    //double trace_XXT_total = 0;
    //MPI_Allreduce(&trace_XXT, &trace_XXT_total, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

    //c_scale = sqrt(Mt / trace_XXT_total);
    c_scale = 1.0 / sqrt(N);
    (*dataset).set_c_scale(c_scale);

    if (rank == 0)
        std::cout << "c_scale = "  << c_scale << std::endl;

    // we scale mixture variances of effects
    for (int i=0; i<vars.size(); i++)
        vars[i] /= (c_scale*c_scale);
...
std::vector<double> vamp::infere_linear(data* dataset){
    ...
    alpha1 = 0;
    p_last = std::vector<double> (4 * (*dataset).get_mbytes(), 0.0);
    std::vector<double> residual(4 * (*dataset).get_mbytes(), 0.0);
    ...
    r2 = std::vector<double> (M, 0.0);
    z2 = std::vector<double> (4 * (*dataset).get_mbytes(), 0.0);

    gam2 = 1.0 / inner_prod(probs, vars, 0);
    // gam2 = 1e-4;
    // gam2 = 0.1;
    ...
    {    
        ...
        if (it > 1){
            std::vector<double> res = lmmse_multAAT(mu_CG_last, gamw, dataset);
            // no need since we save non-scaled version already
            //for (int i=0; i<N; i++)
            //    res[i] = res[i] / gam2 / gamw;
            std::transform (residual.begin(), residual.end(), res.begin(), res.begin(), std::minus<double>());
            std::transform (p_last.begin(), p_last.end(), res.begin(), p_last.begin(), std::plus<double>());
        }

        if (rank == 0)
            std::cout << "l2_norm(p_last, 0) = " << sqrt(l2_norm2(p_last, 0)) << std::endl;
        if (rank == 0)
            std::cout << "l2_norm(mu_CG_last, 0) = " << sqrt(l2_norm2(mu_CG_last, 0)) << std::endl;

        x2_hat = lmmse_denoiserAAT(r2, mu_CG_last, p_last, &correction, &new_vars, dataset);
        ...
        if (it < 0){
            double g2d_correction = g2d_onsager(gam2, gamw, dataset);
            if (rank == 0)
                std::cout << "g2d_correction = " << g2d_correction << std::endl;
            alpha2 = g2d_correction;
            for (int i = 0; i < M; i++)
                r1[i] = (x2_hat[i] - alpha2 * r2[i]) / (1-alpha2);
        }
        else{
            alpha2 = 1 - correction / gam2;
  
            //r1_prev = r1;
            for (int i = 0; i < M; i++)
                r1[i] = r2[i] + x2_hat[i] / correction;
        }
        ...
        gam1 = std::min( std::max( 1.0 / new_vars, gamma_min ), gamma_max );

        for (int i=0; i<M; i++)
            x2_hat[i] = r2[i] + x2_hat[i] / gam2;
        ...
        double se_dev1 = 0;
        for (int i0=0; i0<M; i0++)
            se_dev1 += (r1[i0]- true_signal[i0]/c_scale)*(r1[i0]- true_signal[i0]/c_scale);
        ...
        for (int i0=0; i0<x1_hat_stored.size(); i0++)
            x1_hat_stored[i0] =  x1_hat[i0] * c_scale;
        ...
        for (int i = 0; i < M; i++)
            r2[i] = (x1_hat[i] - alpha1 * r1[i]) / (1 - alpha1);
        
        z2 = (*dataset).Ax(r2.data());
        ...
        gam_before = gam2;
        gam2 = std::min( std::max( (double) Mt / (l2_norm2(residual, 0) - (double) N / gamw), gamma_min), gamma_max);
        ...
        double se_dev = 0;
        for (int i0=0; i0<M; i0++){
            se_dev += (r2[i0] - true_signal[i0]/c_scale)*(r2[i0] - true_signal[i0]/c_scale);
        }
...
double vamp::g2d_onsager(double gam2, double tau, data* dataset) { // shared between linear and binary classification model
    ...
    std::vector<double> invQ_bern_vec1 = (*dataset).Ax(invQ_bern_vec.data());

    double correction_temp = 0;
    double new_vars_temp = 0;

    std::vector<double> init_p = std::vector<double> (4*(*dataset).get_mbytes(), 0.0);

    std::vector<double> invQ_bern_vec2 = CG_solverAAT(invQ_bern_vec1, std::vector<double> (4*(*dataset).get_mbytes(), 0.0), init_p, gamw, &correction_temp, &new_vars_temp, 0, dataset);

    //invQ_bern_vec = precondCG_solver(bern_vec, tau, 0, dataset, redglob); // precond_change

    invQ_bern_vec = (*dataset).ATx(invQ_bern_vec2.data());

    double onsager = inner_prod(bern_vec, invQ_bern_vec, 1); // because we want to calculate gam2 * Tr[(gamw * X^TX + gam2 * I)^(-1)] / Mt
...
void vamp::err_measures(data *dataset, int ind){
    ...
    if (ind == 1){
        for (int i = 0; i< M; i++)
            temp[i] = c_scale * x1_hat[i] - true_signal[i];

        l2_norm2_xhat = l2_norm2(x1_hat, 1) * c_scale * c_scale;
    }
    else if (ind == 2){
        for (int i = 0; i< M; i++)
            temp[i] = c_scale * x2_hat[i] - true_signal[i];

        l2_norm2_xhat = l2_norm2(x2_hat, 1) * c_scale * c_scale;
    }
